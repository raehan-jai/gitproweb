...
@php
    $textsDefault = [
        ...
        'principal_name' => $templateSertifikat->nama_kepsek ?: 'Nama Kepala Sekolah',
        'principal_nip' => $templateSertifikat->nip_kepsek ? 'NIP. ' . $templateSertifikat->nip_kepsek : 'NIP. 000000000000000000',
        'committee_title' => 'Ketua Panitia',
        'committee_name' => $templateSertifikat->nama_panitia ?: 'Nama Ketua Panitia',
        'committee_nip' => $templateSertifikat->nip_panitia ? 'NIP. ' . $templateSertifikat->nip_panitia : 'NIP. 000000000000000000',
    ...
    $stylesDefault = [
        ...
        'participant_name' => ['font_size' => (int) old('font_size_nama', $templateSertifikat->font_size_nama ?? 28) * 1.45],
        'participant_info' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
        'participation_text' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
        'activity_name' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.75],
        'activity_detail' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
        'principal_title' => ['font_size' => 13],
        'principal_name' => ['font_size' => 12],
        'principal_nip' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
        'committee_title' => ['font_size' => 13],
        'committee_name' => ['font_size' => 12],
        'committee_nip' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
        'certificate_number' => ['font_size' => (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05],
    ];

    $texts = array_merge($textsDefault, is_array(old('certificate_texts', $templateSertifikat->certificate_texts ?? [])) ? old('certificate_texts', $templateSertifikat->certificate_texts ?? []) : []);
    $positions = array_replace_recursive($positionsDefault, is_array(old('element_positions', $templateSertifikat->element_positions ?? [])) ? old('element_positions', $templateSertifikat->element_positions ?? []) : []);
    $styles = array_replace_recursive($stylesDefault, is_array(old('element_styles', $templateSertifikat->element_styles ?? [])) ? old('element_styles', $templateSertifikat->element_styles ?? []) : []);
    $perataanSaatIni = old('perataan_teks', $templateSertifikat->perataan_teks ?? 'tengah');
@endphp

<form action="{{ route('template-sertifikat.update', $templateSertifikat) }}" method="POST" enctype="multipart/form-data" id="templateEditorForm">
@csrf
@method('PUT')
...
<div class="card table-card template-preview-card">
    <div class="card-header bg-white py-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
        ...
        <div class="d-flex flex-wrap align-items-center gap-2">
            <span class="text-muted small">Tekan Edit untuk mengubah isi sertifikat</span>
        ...
        <div class="template-editor-wrap">
            <aside class="editor-drawer" id="editorDrawer">
                <div class="editor-drawer-header">
                    <div>
                        <h6 class="mb-0 fw-bold">Form Edit Template</h6>
                        <small class="text-muted">Form ini tampil setelah tombol Edit ditekan.</small>
                    </div>
                ...
                <div class="editor-drawer-body">
                    ...
                    <input type="hidden" name="font_size_nama" value="{{ old('font_size_nama', $templateSertifikat->font_size_nama) }}">
                    <input type="hidden" name="font_size_detail" value="{{ old('font_size_detail', $templateSertifikat->font_size_detail) }}">
                            ...
                            <div id="imageFields" class="accordion-collapse collapse" data-bs-parent="#editorAccordion">
                                <div class="accordion-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="hidden" name="remove_images[background]" id="removeBg" value="0">
                                        <button type="button" class="btn btn-outline-danger btn-sm mt-2 js-remove-image"
                                                data-input="inputBg" data-image="previewBgImg" data-remove="removeBg">
                                            <i class="bi bi-trash me-1"></i>Hapus Background
                                        </button>
                                        <small class="text-muted">JPG/PNG, maks 10MB. Rekomendasi 1123x794px.</small>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="hidden" name="remove_images[logo_sekolah]" id="removeLogoSekolah" value="0">
                                        <button type="button" class="btn btn-outline-danger btn-sm mt-2 js-remove-image"
                                                data-input="inputLogoSekolah" data-image="previewLogoSekolahImg" data-remove="removeLogoSekolah">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="hidden" name="remove_images[ttd_kepsek]" id="removeTtdKepsek" value="0">
                                        <button type="button" class="btn btn-outline-danger btn-sm mt-2 js-remove-image"
                                                data-input="inputTtdKepsek" data-image="previewTtdKepsekImg" data-remove="removeTtdKepsek" data-fallback="TTD Kepala Sekolah">
                                    ...
                                    <div class="mb-0">
                                        ...
                                        <input type="hidden" name="remove_images[ttd_panitia]" id="removeTtdPanitia" value="0">
                                        <button type="button" class="btn btn-outline-danger btn-sm mt-2 js-remove-image"
                                                data-input="inputTtdPanitia" data-image="previewTtdPanitiaImg" data-remove="removeTtdPanitia" data-fallback="TTD Ketua Panitia">
            ...
            <div class="preview-scroll">
                <div class="certificate-canvas" id="certificateCanvas">
                    @if($templateSertifikat->background_path)
                        <img src="{{ Storage::url($templateSertifikat->background_path) }}" class="certificate-bg" id="previewBgImg" alt="Background">
                    @else
                        <img src="" class="certificate-bg d-none" id="previewBgImg" alt="Background">
                    @endif

                    @if($templateSertifikat->logo_sekolah_path)
                        <img src="{{ Storage::url($templateSertifikat->logo_sekolah_path) }}" class="canvas-item logo-item"
                             id="previewLogoSekolahImg" data-element="logo" alt="Logo Sekolah"
                    ...
                    <div class="canvas-item signature-item" data-element="ttd_kepsek"
                        ...
                        @if($templateSertifikat->ttd_kepsek_path)
                            <img src="{{ Storage::url($templateSertifikat->ttd_kepsek_path) }}" id="previewTtdKepsekImg" alt="TTD Kepala Sekolah">
                        @else
                    ...
                    <div class="canvas-item signature-item" data-element="ttd_panitia"
                        ...
                        @if($templateSertifikat->ttd_panitia_path)
                            <img src="{{ Storage::url($templateSertifikat->ttd_panitia_path) }}" id="previewTtdPanitiaImg" alt="TTD Ketua Panitia">
                        @else
...
<style>
    ...
    .text-participant_name {
        font-size: {{ (int) old('font_size_nama', $templateSertifikat->font_size_nama ?? 28) * 1.45 }}px;
        font-weight: 700;
    }
    ...
    .text-principal_nip,
    .text-committee_nip {
        font-size: {{ (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.05 }}px;
        color: #334155;
    }

    .text-activity_name {
        font-size: {{ (int) old('font_size_detail', $templateSertifikat->font_size_detail ?? 14) * 1.75 }}px;
        font-weight: 700;
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (drawer && drawerHeader) {
            ...
            drawerHeader.addEventListener('pointermove', event => {
                ...
                const wrapper = drawer.closest('.template-editor-wrap');
        ...
        const bindImagePreview = (inputId, imageId, fallbackSelector = null) => {
            ...
            const removeInput = document.querySelector(`[data-input="${inputId}"]`)?.dataset.remove;
            ...
            input.addEventListener('change', event => {
                ...
                reader.onload = readerEvent => {
                    ...
                    if (removeInput) document.getElementById(removeInput).value = '0';
        ...
        document.querySelectorAll('.js-remove-image').forEach(button => {
            button.addEventListener('click', () => {
                ...
                const removeInput = document.getElementById(button.dataset.remove);
